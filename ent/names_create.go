...
func (nc *NamesCreate) SetID(u uuid.UUID) *NamesCreate {
	nc.mutation.SetID(u)
	return nc
}

// SetNillableID sets the "id" field if the given value is not nil.
func (nc *NamesCreate) SetNillableID(u *uuid.UUID) *NamesCreate {
	if u != nil {
		nc.SetID(*u)
	}
	return nc
...
func (nc *NamesCreate) Save(ctx context.Context) (*Names, error) {
	...
	if len(nc.hooks) == 0 {
		...
		nv, ok := v.(*Names)
		if !ok {
			return nil, fmt.Errorf("unexpected node type %T returned from NamesMutation", v)
		}
		node = nv
	}
	return node, err
